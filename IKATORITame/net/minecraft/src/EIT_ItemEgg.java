...
public class EIT_ItemEgg extends ItemEgg {
    ...
    @Override
    public ItemStack onItemRightClick(ItemStack pItemstack, World pWorld, EntityPlayer pEntityplayer) {
        if (!pEntityplayer.capabilities.isCreativeMode) {
            --pItemstack.stackSize;
        }

        pWorld.playSoundAtEntity(pEntityplayer, "random.bow", 0.5F, 0.4F / (itemRand.nextFloat() * 0.4F + 0.8F));
