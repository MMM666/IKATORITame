...
public class EIT_EntityAITemptChickenTame extends EntityAITempt {
    ...
    public EIT_EntityAITemptChickenTame(EIT_EntityChicken par1EntityCreature, float par2, int par3, boolean par4, double pStorprange) {
    ...
    public boolean shouldExecute() {
    	if (temptedEntity.attackTime > 0 || temptedEntity.health >= temptedEntity.getMaxHealth()) {
    		return false;
    	}
    	boolean lflag = super.shouldExecute();
    	try {
    		temptingPlayer = (EntityPlayer)ModLoader.getPrivateValue(EntityAITempt.class, this, 7);
    	} catch (Exception e) {
    	}
    	return lflag;
    }
    ...
    public void updateTask() {
        ...
        if (temptedEntity.getDistanceSqToEntity(temptingPlayer) < (temptedEntity.isTamed() ? range : 6.25D)) {
