...
//	private int eatInterval;
	...
	public EIT_EntityAIEatFoodChickenTame(EIT_EntityChicken parEntity, float parDelta) {
		super();
        setMutexBits(3);
...
//		eatInterval = 0;
	...
	public boolean continueExecuting() {
		return (targetEntity != null && targetEntity.isEntityAlive()) || ownerEntity.attackTime > 0;
	}
		...
		if (ownerEntity.getDistanceSqToEntity(targetEntity) < 0.5D) {
    		...
    		if (ownerEntity.attackTime <= 0) {
    			if (ownerEntity.eatBeans()) {
        	        ownerEntity.getLookHelper().setLookPositionWithEntity(targetEntity, 10F, ownerEntity.getVerticalFaceSpeed());
        	        ownerEntity.attackTime = 5;
        			if (--targetEntity.func_92059_d().stackSize <= 0) {
            ...
            ownerEntity.getNavigator().tryMoveToXYZ(targetEntity.posX, targetEntity.posY, targetEntity.posZ, pathDelta);
	        ownerEntity.getLookHelper().setLookPositionWithEntity(targetEntity, 10F, ownerEntity.getVerticalFaceSpeed());
        }
	}
    ...
    {
        float f = 16F;
        List list = theWorld.getEntitiesWithinAABB(EntityItem.class, ownerEntity.boundingBox.expand(f, f, f));

        for (Iterator iterator = list.iterator(); iterator.hasNext();)
        {
            Entity entity = (Entity)iterator.next();
            EntityItem entityitem = (EntityItem)entity;
            ItemStack litemstack = entityitem.func_92059_d();
