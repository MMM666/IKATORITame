...
//		System.out.println("tamedSquid.");
    ...
    public void setMyFlag(int pindex, boolean pflag) {
        ...
        if (pflag) {
            this.dataWatcher.updateObject(16, Byte.valueOf((byte)(lval | pindex)));
        } else {
            this.dataWatcher.updateObject(16, Byte.valueOf((byte)(lval & ~pindex)));
        }
    }
...
;                   	}
                ...
                if (isTamed() && itemstack.itemID == Item.dyePowder.itemID && itemstack.getItemDamage() == 0) {
        			// RIDE-ON
        			// イカに乗る
                    if (!entityplayer.capabilities.isCreativeMode) {
                        if (--itemstack.stackSize <= 0) {
    ...
    public void squidUpdateEntityActionState() {
        ...
        if(list != null) {
            for(int i = 0; i < list.size(); i++) {
                ...
                boolean lflag = this.worldObj.handleMaterialAcceleration(entity1.boundingBox, Material.water, entity1);
                if(!entity1.isDead && lflag && (entity1 instanceof EntityItem) && ((EntityItem)entity1).func_92059_d().itemID == Item.fishRaw.itemID) {
            ...
            if(entity != null && d < 1.0D && ((EntityItem)entity).delayBeforeCanPickup <= 0 && eatFish()) {
            	// アイテムに接触
            	if (--((EntityItem)entity).func_92059_d().stackSize <= 0) {
                	entity.setDead();
            	}
                entity = null;
            }
        ...
        if(entity != null) {
        	try {
        		float randomMotionSpeed = (Float)ModLoader.getPrivateValue(EntitySquid.class, this, 8);
            	if(randomMotionSpeed == 1.0F) {
            		...
            		ModLoader.setPrivateValue(EntitySquid.class, this, 11, Float.valueOf(randomMotionVecX));
            		ModLoader.setPrivateValue(EntitySquid.class, this, 12, Float.valueOf(randomMotionVecY));
            		ModLoader.setPrivateValue(EntitySquid.class, this, 13, Float.valueOf(randomMotionVecZ));
                }
        	}
        	catch (Exception e) {
        	}
