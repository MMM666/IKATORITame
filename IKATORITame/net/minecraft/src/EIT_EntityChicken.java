...
public class EIT_EntityChicken extends EntityChicken {
    ...
    public String getTexture() {
    	// ひよこのテクスチャは別
   		return (mod_EIT_IKATORITame.isChickTexture && isChild()) ? textureChick : fFullFrontal ? textureChickenFrontal : texture;
    }
    ...
    public boolean interact(EntityPlayer entityplayer) {
        return chickenInteract(entityplayer);
    }

	@Override
    public void onLivingUpdate() {
        if(!worldObj.isRemote) {
            chickenOnLivingUpdate();
        }        
    ...
    public boolean chickenInteract(EntityPlayer entityplayer) {
    	// なでり判定
        if(entityplayer != null) {
            ...
            if(itemstack != null && itemstack.stackSize > 0) {
                ...
                if(isTamed()) {
                    ...
                    if (itemstack.itemID == Item.feather.itemID) {
            			// RIDE-ON
            			// トリに乗る
                        if (!entityplayer.capabilities.isCreativeMode) {
                            if (--itemstack.stackSize <= 0) {
    ...
    public void chickenOnLivingUpdate() {
        // Ride-onで接地判定
        if(isRiding() && mod_EIT_IKATORITame.isChickenGride) {
            // グライド判定
            Entity entity;
